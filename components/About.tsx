"use client";

import { useLocale } from "next-intl";
import { company, sections, t, type Locale } from "@/data/content";
import { FadeIn } from "@/components/motion";
import { ImagePlaceholder } from "@/components/ImagePlaceholder";
import { Badge } from "@/components/ui/badge";

export function About() {
  const locale = useLocale() as Locale;

  return (
    <section id="about" className="section-padding">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <h2 className="section-title-gap font-display text-3xl font-bold sm:text-4xl rtl:text-right">
            {t(sections.about, locale)}
          </h2>
        </FadeIn>

        <div className="grid items-center gap-4 md:gap-6 lg:grid-cols-2">
          <FadeIn className="rtl:text-right">
            <Badge variant="sand" className="mb-4 rtl:float-end">
              {t(company.locationBadge, locale)}
            </Badge>
            <p className="mb-4 clear-both text-[18px] leading-[1.8] text-muted">
              {t(company.aboutParagraph, locale)}
            </p>
            <p className="text-[18px] leading-[1.8]">
              <span className="highlight-amber font-medium">
                {t(company.aboutHighlight, locale)}
              </span>
            </p>
          </FadeIn>

          <FadeIn delay={0.15}>
            <ImagePlaceholder
              src="/images/about-team.jpg"
              filename="about-team-photo.jpg"
              alt={t(company.name, locale)}
              usePlaceholder={true}
              className="rounded-xl"
              aspectClass="min-h-[380px]"
            />
          </FadeIn>
        </div>
      </div>
    </section>
  );
}
